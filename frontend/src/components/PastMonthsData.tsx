...
import { Search, Calendar, BarChart3, Fuel } from 'lucide-react';
...
const PastMonthsData: React.FC<PastMonthsDataProps> = ({ selectedMonth }) => {
  ...
  const loadMonthData = async () => {
    try {
      ...
      const dailyResponse = await dataAPI.getAll({ month: selectedMonth });
      setDailyData(dailyResponse.data.data || []);
      
      // Load monthly summary for the selected month
      const monthlyResponse = await dataAPI.getMonthly({ month: selectedMonth });
